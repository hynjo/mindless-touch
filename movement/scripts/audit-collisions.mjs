import {applyHandOffsets,blendHandOffsets} from '../hand-offsets.js';
...
function inspect(r,name){
 ...
 return {name,wristLimitViolations:r.self.jointViolations(),contacts:contacts.map(({a,b,depthMm})=>({a,b,depthMm})),surfaceClearanceMm:Math.round(clearance*1e6)/1000};
}
...
for(const [name,steps,r] of [['Sun Salutation',sunSalutation.steps,yoga],['Pole Flow',poleFlow.steps,pole]]){
 ...
 apply(r,steps[0]);
 const timeline=createTimeline(steps),collisions=new Map();let samples=0,surfaceFailures=0,wristLimitFailures=0;
 ...
 for(let t=0;t<=timeline.duration;t+=.05){
  ...
  else{blendHandOffsets(r.root,a.handOffsets,b.handOffsets,s.mix);r.root.position.lerpVectors(a.position,b.position,s.mix);r.joints.forEach((j,i)=>j.group.quaternion.slerpQuaternions(a.rotations[i],b.rotations[i],s.mix));if(palmsAreSupported(steps[s.index])&&palmsAreSupported(steps[s.next]))placePalmsOnFloor(r);else if(palmsAreSupported(steps[s.index])||palmsAreSupported(steps[s.next]))preparePalmLanding(r);r.floor.settle();}
  const result=inspect(r,`${steps[s.index].name} → ${steps[s.next].name}`);samples++;
  if(result.surfaceClearanceMm<-.1)surfaceFailures++;
  if(result.wristLimitViolations.length)wristLimitFailures++;
 ...
 examples[name]={cards,samples,surfaceFailures,wristLimitFailures,collisions:[...collisions.values()]};
}
const summary={total:library.length,wristLimitFlagged:library.filter(p=>p.wristLimitViolations.length).length,flagged:library.filter(p=>p.contacts.length).length,clear:library.filter(p=>!p.contacts.length).length,surfaceFailures:library.filter(p=>p.surfaceClearanceMm<-.1).length};
...
md+='\n## Wrist limits\n\n'+summary.wristLimitFlagged+' library poses exceed the editor wrist direction limits. These require separate arm/hand alignment review; they are not silently corrected.\n\n'+library.filter(p=>p.wristLimitViolations.length).map(p=>p.name+': '+p.wristLimitViolations.map(v=>v.id).join(', ')).join('\n\n')+'\n';
...
for(const [name,e] of Object.entries(examples)){
 md+=`\n## ${name}\n\n${e.cards.length} cards, ${e.cards.filter(c=>c.contacts.length).length} flagged cards. ${e.samples} transition/hold samples; ${e.surfaceFailures} surface failures; ${e.wristLimitFailures} samples exceed wrist limits.\n\n| Transition | Parts | Time interval (s) | Maximum depth (mm) |\n| --- | --- | --- | ---: |\n`;
...
console.log(JSON.stringify({summary,examples:Object.fromEntries(Object.entries(examples).map(([name,e])=>[name,{cards:e.cards.length,flaggedCards:e.cards.filter(c=>c.contacts.length).length,collisions:e.collisions.length,surfaceFailures:e.surfaceFailures,wristLimitFailures:e.wristLimitFailures}]))},null,2));
