...
function pose(id, name, cue, {height=.97, pelvis=0, chest=0, head=0, hip=0, knee=0, shoulder=0, elbow=0, ankle=0} = {}) {
  const rotations = {pelvis:[pelvis,0,0],torso:[chest,0,0],neck:[head,0,0]};
  for (const side of ['left','right']) {
    for (const [joint,angle] of Object.entries({Hip:hip,Knee:knee,Shoulder:shoulder,Elbow:elbow,Ankle:ankle})) rotations[side+joint] = [angle,0,0];
  }
  return {id,name,cue,holdSeconds:1.4,transitionSeconds:2.2,rootPosition:[0,height,0],rotations};
}
const mountain = {};
const salute = {shoulder:-180,head:-12};
const fold = {pelvis:155,hip:-155,shoulder:-155,head:10};
const halfLift = {pelvis:90,hip:-90,shoulder:-90,head:-30};
function transition(id,name,forward) {
  ...
  step.rotations.leftHip = [-100,0,0];step.rotations.leftKnee = [105,0,0];
...
export const sunSalutation = {
  ...
  steps:[
    ...
    {...pose('plant-hands','Plant Hands','Prepare to step',{height:.76,pelvis:120,hip:-135,knee:45,shoulder:-120}),kind:'transition',holdSeconds:.5,transitionSeconds:2.4},
    ...
    pose('upward-dog','Upward-Facing Dog','Inhale',{height:.22,pelvis:65,chest:-15,hip:15,shoulder:-50,head:-20}),
    pose('downward-dog','Downward-Facing Dog','Hold',{height:.8,pelvis:125,hip:-80,shoulder:-180,head:15,ankle:-45}),
    transition('step-forward','Step Forward',true),
    {...pose('step-together','Step Together','Bring feet forward',{height:.76,pelvis:120,hip:-135,knee:45,shoulder:-120}),kind:'transition',holdSeconds:.5,transitionSeconds:2.4},
