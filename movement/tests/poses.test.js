...
test('all library poses clear the floor and nonadjacent body volumes',()=>{
 ...
 assert.equal(new Set(yogaPoses.map(p=>p.id)).size,yogaPoses.length);
 assert.equal(new Set(yogaPoses.map(p=>p.source)).size,yogaPoses.length);
 assert.equal(yogaPoses.length,171);
 for(const pose of yogaPoses){
  ...
  if(pose.name==='Lizard') {
   ...
   assert(knee.rotation.x>=0 && knee.rotation.x<Math.PI*5/6,'Trailing knee may be straight or flex forward, never hyperextend');
...
test('Cat–Cow bends pelvis, lumbar and thoracic joints while palms and knees stay anchored',()=>{
 ...
 const meshes=[];body.root.traverse(mesh=>{if(mesh.isMesh)meshes.push(mesh);});
 const rig={...body,byId,dimensions,meshes};
 ...
 for(let frame=0;frame<=40;frame++){
  ...
  assert.deepEqual(inspectSupports(rig,{supportRequirements:['palms','knees']}).issues,[],`Floor support failed at ${frame}`);
  assert(meshes.every(mesh=>floorClearance(mesh)>=FLOOR_SKIN-.00002),`Floor penetration at ${frame}`);
